...
router.get('/new', isAuthenticated, canManageMembersVisits, async (req, res) => {
    try {
        ...
        res.render('log-visit', {
            ...
            ticketTypes: ticketTypes, activeMembers: activeMembers, currentDiscount: currentDiscount,
            normalizePhone: normalizePhone, foundTickets: foundTickets, ticketSearchTerm: ticketSearchTerm
        });
        req.session.error = null; req.session.success = null;
    } catch (error) {
        res.render('log-visit', { error: "Error fetching data.", success: null, ticketTypes: [], activeMembers: [], currentDiscount: 0, normalizePhone: (p) => p, foundTickets: null, ticketSearchTerm: "" });
    }
});
...
router.post('/', isAuthenticated, canManageMembersVisits, async (req, res) => {
    ...
    try {
        ...
        if (ticket.is_member_type) {
            ...
            const guestTickets = []; // For Carousel
            ...
            for (const memberId of member_ids) {
                ...
                if (ticket.type_name === 'Guest Pass') {
                    ...
                    for (let i = 0; i < qtyRequested; i++) {
                        ...
                        guestTickets.push({
                            name: "Guest Pass",
                            code: `${member.public_membership_id.substring(0, 8)}-G${i + 1}`, // Mock code for display
            ...
            let receiptData = {
                ...
                // Default these to first member for header
            ...
            if (ticket.type_name === 'Guest Pass') {
                ...
                // Standard Member Receipt logic (List view)
...
router.post('/redeem', isAuthenticated, canManageMembersVisits, async (req, res) => {
    ...
    try {
        ...
        if (ticketResult.length === 0) { req.session.error = `Ticket code "${ticket_code}" not found.`; return res.redirect('/visits/new'); }
        const ticket = ticketResult[0];
        if (ticket.is_redeemed) { req.session.error = `Already redeemed.`; return res.redirect('/visits/new'); }
        await connection.beginTransaction();
        const visitGroupId = crypto.randomUUID();
        const visit_date = new Date();
        ...
        let receiptData = {
            ...
            tickets: [{ name: ticketType[0].type_name, code: ticket_code }] // For Carousel
        };
        res.render('visit-receipt', { receipt: receiptData, fromLogVisit: true });
    } catch (error) {
        if (connection) await connection.rollback();
        req.session.error = "Error during redemption."; res.redirect('/visits/new');
