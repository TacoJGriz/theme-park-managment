...
BEGIN
    ...
    -- OPTIMIZATION: Get max row counts ONCE
    ...
    WHILE i < num_visits DO
        ...
        -- Select a random date based on weighted score (this query is already efficient)
        ...
        IF is_member THEN
            -- OPTIMIZATION: Assign a random member by indexed ID
            IF max_member_row_id > 0 THEN
                SET random_row_id = FLOOR(1 + RAND() * max_member_row_id);
                SELECT membership_id INTO member_id
                FROM temp_active_members
                WHERE id = random_row_id;
            END IF;
        ELSE
            -- Check for promotions for non-members
            ...
            END IF;
            -- Note: The original script set final_price = final_price - discount, 
            -- but the DB schema seems to store ticket_price as final_price. 
            -- Re-instating this line from the original script logic:
            SET final_price = final_price - discount; 
        END IF;

        -- OPTIMIZATION: Get a random, active 'Staff' employee by indexed ID
        IF max_staff_row_id > 0 THEN
            SET random_row_id = FLOOR(1 + RAND() * max_staff_row_id);
            SELECT employee_id INTO v_employee_id
            FROM temp_staff_employees
            WHERE id = random_row_id;
        ELSE
            SET v_employee_id = NULL; -- No staff available
        END IF;
